...
export default class TransactionScreen extends Component {
  ...
  checkBookAvailability = async bookId => {
    ...
    {
        var checking = await db.collection('students').where('student_id', '==' , studentId).get()
        var isStudentEligible = ''

        if(checking.doc.length == 0)
        {
            this.setState({bookId:'',studentId:''})
        ...
        else
        {
            checking.docs.map(doc => 
            {
                ...
                {
                    isStudentEligible = false
                    Alert.alert('The student has already been issued 2 books')
                    this.setState({bookId:'',studentId:''})
                }
            })
        }
